...
export class BuildersForSales {
  ...
  public async CreateProductsSold(
    createProductsSoldDTO: CreateProductsSoldDTO[],
  ): Promise<ProductsSoldEntity[]> {
    const building = createProductsSoldDTO.map(async (productSold) => {
      const productData = await this.productService.findProductAllData(
        productSold.id,
      );

      if (productData.amount < productSold.amount) {
        throw new HttpException(
          `Not enough items in stock for ${productSold.amount} ${productData.name} (id: ${productSold.id})`,
  ...
  public async proceduresPerformed(
    proceduresPerformedDTO: CreateProductsSoldDTO[],
  ): Promise<ProceduresPerformedEntity[]> {
    const building = proceduresPerformedDTO.map(
      async (updateProcedurePerformed) => {
        const procedureData =
          await this.procedureService.findOneProcedureAllData(
            updateProcedurePerformed.id,
          );

        const priceProcedurePerformed =
          procedureData.price * updateProcedurePerformed.amount;
