...
export class SalesService {
  constructor(
    ...
    @InjectRepository(ProductsSoldEntity)
    private readonly productsSoldRepository: Repository<ProductsSoldEntity>,
  ) {}
  ...
  public async createSale(
    ...
    if (!createSaleDto.proceduresPerformed && !createSaleDto.productsSold)
      throw new HttpException(
        `the sale needs at least one proceduresPerformed or productsSold`,
        HttpStatus.BAD_REQUEST,
      );

    const buildproceduresPerformedSale = () => {
      if (createSaleDto.proceduresPerformed)
        return this.buildersForSales.proceduresPerformed(
          createSaleDto.proceduresPerformed,
        );
    };
    const buildProductSoldSale = () => {
      if (createSaleDto.productsSold)
        return this.buildersForSales.CreateProductsSold(
    ...
    const priceSale = this.buildersForSales.CreateSalePrice(
      proceduresPerformedSale,
      productSoldSale,
    );
  ...
  public async deleteSale(id: string): Promise<void> {
    ...
    await Promise.all(
      sale.productsSold.map(async (productsSold) => {
        const productSoldAllData = await this.productsSoldRepository.findOne({
          where: { id: productsSold.id },
          relations: { product: true },
        });
        return this.productService.updateProduct(
          productSoldAllData.product.id,
          { amount: productSoldAllData.product.amount + productsSold.amount },
        );
      }),
    );
